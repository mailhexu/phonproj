...
def calculate_displacement(mass, wavenumber, eigenvector, temperature, N=1, factor=1):
    ...
    n = bose_einstein_distribution(wavenumber, temperature, verbose=True)
...
def calculate_thermal_displacement_squared(mass, wavenumber, eigenvector, temperature, N=1):
    """
...
if __name__ == "__main__":
    ...
    eigenvectors = (np.random.random((3, 3, 3)) + 1j * np.random.random((3, 3, 3))) / np.sqrt(2)
    temperature = 300  # Room temperature in K
    
    # Calculate displacements
    print("\nExample calculation at room temperature:")
    # Calculate mean square displacements
    displacements_squared = calculate_total_displacement(masses, wavenumbers, eigenvectors, temperature)
